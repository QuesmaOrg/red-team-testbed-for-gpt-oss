...
class BaseTester(ABC):
    ...
    def run_single_test(self, test: BaseTest, current_num: int = 1, total_tests: int = 1) -> Tuple[BaseTest, List[ModelResponse], EvaluationResult]:
        ...
        try:
            if test.multi_turn and (test.follow_up_prompts or test.conversation_turns):
                ...
                if test.conversation_turns:
                    ...
                    for i, turn in enumerate(test.conversation_turns, 1):
                        ...
                        display.show_prompt(progress, user_msg, test.system_prompt if i == 1 else "")
                        display.start_thinking_timer(progress)
                        
                        response = self.client.generate(user_msg, test.system_prompt if i == 1 else "")
            ...
            if any(r.error for r in responses):
                ...
                evaluation = EvaluationResult(
                    False, self.get_vulnerability_category(), 0.0, [], 
    ...
    def _evaluate_test_response(self, test: BaseTest, responses: List[ModelResponse]) -> EvaluationResult:
        """
    ...
    def run_category_tests(self, category: str = None, test_id: str = None) -> List[Tuple[BaseTest, List[ModelResponse], EvaluationResult]]:
    ...
    def analyze_results(self, results: List[Tuple[BaseTest, List[ModelResponse], EvaluationResult]]) -> Dict[str, Any]:
...
def run_category_tests_generic(tester_class, client: OllamaClient, category: str = None, test_id: str = None) -> Dict[str, Any]:
