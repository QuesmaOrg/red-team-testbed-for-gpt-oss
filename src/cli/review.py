...
def select_results_file_interactive(results_dir: str = "results") -> str | None:
    ...
    for file_info in files_info:
        ...
        click.echo(
            f"{file_info['index']:<3} {file_info['filename']:<30} {file_info['size']:<8} {file_info['age']:<12} {file_info['test_count']:<7} {categories_str}"
        )
    ...
    try:
        while True:
            choice = click.prompt(f"\nSelect file (1-{len(files_info)}) or 'q' to quit", type=str)
...
def main(results_file: str | None, config: str, latest: bool) -> int | None:
