...
def list_sessions(manager: SessionManager) -> None:
    ...
    for i, session in enumerate(sessions[:20], 1):
        ...
        table.add_row(
            ...
            f"{success_rate:.0%}",
            session["target_model"].split("/")[-1]
            if "/" in session["target_model"]
            else session["target_model"],
        )
...
def view_lessons(manager: SessionManager) -> None:
    ...
    idx = IntPrompt.ask(
        "\nSelect model", default=1, choices=[str(i) for i in range(1, len(lesson_files) + 1)]
    )
