...
from src.models import AttackStrategy, AttackAttempt
...
class AdaptiveAttacker:
    ...
    def run_adaptive_campaign(
        self,
        num_attempts: int = 5,
        base_strategy: Optional[AttackStrategy] = None,
        ...
        with Progress(
            ...
            task = progress.add_task(
                "[cyan]Running adaptive campaign...",
                total=num_attempts
            )
            ...
            for attempt_num in range(1, num_attempts + 1):
                ...
                try:
                    ...
                    if attempt.success:
                        ...
                        console.print(f"[green]✓ Success![/green] Confidence: {attempt.evaluation.confidence:.0%}")
                        ...
                        console.print(f"[red]✗ Failed[/red] - Learning from this...")
    ...
    def _adapt_approach(
        self,
        attempt_num: int,
        base_strategy: Optional[AttackStrategy],
        initial_goal: Optional[str],
        steps_range: tuple[int, int],
        vary_approach: bool,
    ) -> tuple[Optional[AttackStrategy], Optional[str], int]:
        ...
        if self.successful_patterns:
            # Try variations of successful patterns
            if random.random() < 0.7:  # 70% chance to exploit success
                return self._exploit_success_pattern()
        
        if self.failed_patterns and random.random() < 0.3:  # 30% chance to retry failed with variation
    ...
    def _vary_goal(self, goal: str, attempt_num: int) -> str:
        ...
        if self.successful_patterns:
            variations.append(f"{goal} (incorporating successful pattern: {self.successful_patterns[-1][:50]})")
        
        return variations[attempt_num % len(variations)]
    
    def _exploit_success_pattern(self) -> tuple[Optional[AttackStrategy], Optional[str], int]:
        """Build on successful patterns."""
        last_success = next(
            (a for a in reversed(self.run_history) if a.success),
            None
        )
    ...
    def _retry_failed_with_variation(self) -> tuple[Optional[AttackStrategy], Optional[str], int]:
        """Retry a failed approach with variations."""
        last_failure = next(
            (a for a in reversed(self.run_history) if not a.success),
            None
        )
    ...
    def _record_success(self, attempt: AttackAttempt) -> None:
        ...
        if attempt.turns:
            exploit_turn = next(
                (t for t in attempt.turns if t.is_exploit_turn),
                attempt.turns[-1]
            )
    ...
    def _learn_from_attempt(self, attempt: AttackAttempt, attempt_num: int) -> None:
        ...
        if attempt.success and attempt.evaluation:
            ...
            if attempt.evaluation.evidence:
                console.print(f"[green]Key success factors:[/green]")
                for evidence in attempt.evaluation.evidence[:3]:
                    console.print(f"  • {evidence}")
            
            # Store for future use
            insight = f"Attempt {attempt_num}: {attempt.strategy.value} with {attempt.steps} steps worked"
            console.print(f"[dim]Insight: {insight}[/dim]")
        
        elif not attempt.success:
            # Why did this fail?
            console.print(f"[yellow]Analyzing failure...[/yellow]")
    ...
    def _show_campaign_summary(self, results: dict) -> None:
        ...
        success_rate = results["successful"] / results["total_attempts"] if results["total_attempts"] > 0 else 0
        ...
        if self.strategy_performance:
            ...
            for strategy, stats in self.strategy_performance.items():
                ...
                console.print(f"  {strategy.value}: {rate:.0%} ({stats['success']}/{stats['total']})")
