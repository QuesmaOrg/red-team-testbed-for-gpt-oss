...
Calls to these tools must go to the commentary channel: 'functions'."""
    ...
    def __init__(self) -> None:
        if not HARMONY_AVAILABLE:
            raise ImportError(
                "openai-harmony library is required for competition format conversion"
            )

    def convert_finding_to_competition(
        self, finding_data: dict[str, Any], user_metadata: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def _build_harmony_conversation(self, finding_data: dict[str, Any]) -> str:
        ...
        messages.append(Message.from_role_and_content(Role.SYSTEM, self.HARMONY_SYSTEM_PROMPT))
    ...
    def _build_competition_json(
        self, finding_data: dict[str, Any], harmony_walkthrough: str, metadata: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "issue_title": metadata.get(
                "issue_title", finding_data.get("test_name", "Security vulnerability detected")
            ),
            "model": {
                "name": model_name,
                "version": "2025-08-05",
            ...
            "issue_summary": {
                "title": metadata.get(
                    "summary_title", finding_data.get("test_name", "Security Issue")
                ),
                ...
                "self_assessed_breadth_justification": metadata.get(
                    "breadth_justification", "Limited to specific scenario tested"
                ),
            },
            "harmony_response_walkthroughs": [harmony_walkthrough],
            "steps_to_reproduce": metadata.get(
                "reproduction_steps",
                [
    ...
    def convert_export_file_to_competition(
        self, export_filepath: str | Path, output_dir: str | Path = "findings"
    ) -> list[str]:
    ...
    def validate_competition_format(
