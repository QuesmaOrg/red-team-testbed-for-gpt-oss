...
class CompetitionSchemaValidator:
    ...
    def validate_data(self, data: Dict[str, Any], file_path: str = "data") -> Tuple[bool, List[str]]:
        """
    ...
    def _format_validation_error(self, error: ValidationError, file_path: str) -> str:
        ...
        if error.validator == "required":
            ...
            max_len = error.schema.get("maxLength", "unknown")
            ...
            min_len = error.schema.get("minLength", "unknown")
            actual_len = len(str(error.instance)) if error.instance else 0
            ...
            min_items = error.schema.get("minItems", "unknown")
            ...
            max_items = error.schema.get("maxItems", "unknown")
            actual_items = len(error.instance) if isinstance(error.instance, list) else 0
            return f"{file_path}: Array too long at '{field_path}'. Max {max_items} items, got {actual_items}"
        elif error.validator == "format":
            expected_format = error.schema.get("format", "unknown")
            return f"{file_path}: Invalid format at '{field_path}'. Expected {expected_format} format"
    ...
    def validate_directory(self, directory_path: str, pattern: str = "*.json") -> Dict[str, Tuple[bool, List[str]]]:
        """
        ...
        if not json_files:
            return {directory_path: (False, [f"No JSON files found matching pattern '{pattern}' in {directory_path}"])}
...
def create_validation_summary(results: Dict[str, Tuple[bool, List[str]]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "error_summary": all_errors[:10] if len(all_errors) > 10 else all_errors,  # Show first 10 errors
