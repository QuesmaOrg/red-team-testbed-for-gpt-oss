...
def extract_prompts_from_findings(findings_dir: str = "findings") -> dict[str, Any]:
        ...
        try:
            ...
            for finding in findings:
                ...
                if conversation_turns and isinstance(conversation_turns, list):
                    ...
                    conversation_prompt = " | ".join(
                        [turn.strip() for turn in conversation_turns if turn and turn.strip()]
                    )
    ...
    return {
        ...
        "reuse_ratio": (len(all_prompts) - len(unique_prompts)) / len(all_prompts)
        if all_prompts
        else 0,
...
def main(
    results_file: str, findings_dir: str, output_format: str, save: str, show_prompts: bool
) -> None:
    ...
    if output_format == "json":
        ...
        if report_data.get("test_results_summary") and not report_data["test_results_summary"].get(
            "error"
        ):
            ...
            if results_stats["latest_session"]:
                ...
                click.echo(
                    f"  • Tests: {latest['tests']} tests, {latest['vulnerable']} vulnerable ({vuln_rate:.1f}%)"
                )
                if latest["categories"]:
                    categories_str = ", ".join(latest["categories"])
                    click.echo(f"  • Categories: {categories_str}")

        elif report_data.get("test_results_summary") and report_data["test_results_summary"].get(
            "error"
        ):
        ...
        if report_data.get("prompt_analysis") and not report_data["prompt_analysis"].get("error"):
            ...
            results_note = (
                f" ({prompt_stats['results_files_available']} results files available)"
                if prompt_stats["results_files_available"] > 0
                else ""
            )
            ...
            prompt_breakdown = (
                f"{total_prompts} ({single_turn} single-turn, {multi_turn} multi-turn conversations)"
                if total_prompts > 0
                else "0"
            )
